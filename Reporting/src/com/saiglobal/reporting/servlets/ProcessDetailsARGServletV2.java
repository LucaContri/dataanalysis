...
public class ProcessDetailsARGServletV2 extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(ProcessDetailsARGServletV2.class);
	private static DbHelperConnPool db;
	private static SfHelper sf;
	private static final GlobalProperties gp;
	private static ProcessDetailsCacheARGv2 argProcessCache;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<String> revenueOwnerships = new ArrayList<String>();
		...
		try {
			...
			try {
				...
				if (parameters.containsKey("q") && (parameters.get("q")[0] != "")) {
					...
					for (String q : qs) {
						if (parametersCache.isRevenueOwnership(q)) {
							revenueOwnerships.add(q);
							continue;
						}
						if (parametersCache.isPathway(q)) {
							pathways.add(q);
							continue;
						}
						...
						SimpleParameter program = parametersCache.getProgramById(q);
						if (program != null) {
			...
			argProcessCache = ProcessDetailsCacheARGv2.getInstance(db);
			
			ProcessDetails data = argProcessCache.getProcessDetails(process, pathways, programs, standards, revenueOwnerships, resources, tags, forceRefresh);
