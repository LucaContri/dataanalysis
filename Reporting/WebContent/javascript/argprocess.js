				...
				function() {
					...
					$("#parameters_input")
							.tokenInput(
									...
									{
										theme : "process",
										hintText : "Filter results by typing any client ownership, resource, standards... "
									});
...
function updateProcessData() {
	...
	if (q !="") {
		...
		var url = "argprocessdetails?process=" + selectedNode + "&q=" + $("#parameters_input").val();
...
function updateProcessSummary() {
	...
	if (selectedNode=="ARG") {
		if ((apiResponse.performances != null) && (apiResponse.performances.length > 0)) {
			...
			var chart = new google.visualization.AreaChart(document.getElementById("process_summary"));
