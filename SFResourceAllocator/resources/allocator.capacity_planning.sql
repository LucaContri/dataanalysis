# Australia Food
set @batchId='Australia Food Capacity Planning - 6 months';
...
#select @batchId, @subBatchId, @startDate, @endDate;

select t.Id,t.Name,t.Work_Item_Date__c, date_format(t.Work_Item_Date__c, '%Y %m') as 'Work Item Period', t.`Status__c` as 'Work Item Status', t.Scheduling_Ownership__c, t.Source, t.`Scheduling Type`, t.Status, t.`Duration Audit (Hrs)`, t.`Duration Travel (Hrs)`, t.`Auditor Reporting Business Unit`, t.`Resource_Type__c` as 'Reource Type', t.`Auditor Name`, t.`Auditor Id`, t.Role__c, t.Comments__c, sp.Standard_Service_Type_Name__c as 'Primary Standard', group_concat(distinct spf.Standard_Service_Type_Name__c) as 'FoS', group_concat(distinct code.Name) as 'Codes' from
...
sum(if(wir.Work_Item_Type__c not in ('Travel'), wir.Total_Duration__c,0)) as 'Duration Audit (Hrs)', if(wir.Work_Item_Type__c in ('Travel'), wir.Total_Duration__c,0) as 'Duration Travel (Hrs)', r.Name as 'Auditor Name', r.Id as 'Auditor Id', wir.Role__c, wi.Comments__c, r.Reporting_Business_Units__c as 'Auditor Reporting Business Unit', r.Resource_Type__c
...
where wi.Work_Item_Date__c between date_format(@startDate, '%Y-%m-%d') and date_format(@endDate, '%Y-%m-%d')
...
group by t.`Id`, t.`Auditor Id`
union all
select s.WorkItemId, s.WorkItemName, s.`StartDate`, date_format(s.`StartDate`, '%Y %m') as 'Work Item Period', 'Pipeline' as 'Status', 'n/a' as 'Scheduling Ownership', 'COMPASS_PIPELINE' as 'Source', 'ALLOCATOR' as 'Scheduling Type', s.`Status`, sum(if(s.`Type`='AUDIT', s.`Duration`,0)) as 'Duration Audit (Hrs)', sum(if(s.`Type`='TRAVEL', s.`Duration`,0)) as 'Duration Travel (Hrs)', r.Reporting_Business_Units__c, r.Resource_Type__c, s.`ResourceName` as 'Auditor Name', s.`ResourceId` as 'Auditor Id', '', s.`Notes`, s.`PrimaryStandard`, null as 'FoS', s.`Competencies` as 'Codes'
...
select asb.BatchId, asb.subBatchId, asb.ScoreAvailabilityDayReward, asb.ScoreCapabilityAuditPenalty, asb.ScoreContractorPenalties, asb.ScoreDistanceKmPenalty, asb.`comment` as 'Allocation Estimated Cost',
count(if(s.Status='NOT_ALLOCATED', s.Id, null)) as 'Not Allocated',
count(if(s.Status='ALLOCATED', s.Id, null)) as 'Allocated'
from salesforce.allocator_schedule s 
inner join salesforce.allocator_schedule_batch asb on s.BatchId = asb.BatchId and s.SubBatchId = asb.SubBatchId
where asb.BatchId=@batchId
and asb.SubBatchId >=10
group by asb.SubBatchId
order by `Not Allocated` asc;

# United Kingdom
set @batchId='UK Food Capacity Planning - 6 months';
set @subBatchId = (select max(SubBatchId) from salesforce.allocator_schedule_batch where BatchId=@batchId);
set @startDate = (select startDate from salesforce.allocator_schedule_batch where BatchId=@batchId and SubBatchId=@subBatchId);
set @endDate = (select endDate from salesforce.allocator_schedule_batch where BatchId=@batchId and SubBatchId=@subBatchId);
...
left join salesforce.resource__c r on s.`ResourceId` = r.Id
where 
wi.Work_Item_Date__c between date_format(@startDate, '%Y-%m-%d') and date_format(@endDate, '%Y-%m-%d')
and wi.Status__c in ('Open', 'Service Change', 'Initiate Service', 'Draft')
and wi.Revenue_Ownership__c = 'EMEA-UK'
and wi.Work_Item_Stage__c not in ('Follow Up')
group by wi.Id) t
