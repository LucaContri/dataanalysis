...
# Usage
Call AuditorsAuditsMatrix();
drop table AuditorsAuditsMatrixData;
select * from AuditorsAuditsMatrixData;

DROP procedure AuditorsAuditsMatrix;
DELIMITER //
#CREATE PROCEDURE AuditorsAuditsMatrix(IN rev_own_like VARCHAR(255), IN periodFrom VARCHAR(7), IN periodTo VARCHAR(7))
CREATE PROCEDURE AuditorsAuditsMatrix()
 BEGIN
 
 declare rev_own_like varchar(255);
 declare periodFrom varchar(7);
 declare periodTo varchar(7);
 set rev_own_like = 'AUS-Food-NSW/ACT';
 set periodFrom = '2016-06';
 set periodTo = '2016-06';
# Temporary tables for rotational requirements
drop table if exists rot_req_audits;
create temporary table rot_req_audits as (
...
drop temporary table if exists rot_req_audits_hist;
create temporary table rot_req_audits_hist as (
...
#explain
drop table if exists AuditorsAuditsMatrixData;
create table AuditorsAuditsMatrixData as (
select mat.*, da.`Days Available`, if (((mat.BRC=1 and rr.BRC=1) or (mat.SQF=1 and rr.SQF=1) or (mat.WQA=1 and rr.WQA=1)) and rr.ResourceIdNotAllowed=mat.`Resource Id`, 1,0) as 'RotReqNotAllowed'
from (
SELECT 
