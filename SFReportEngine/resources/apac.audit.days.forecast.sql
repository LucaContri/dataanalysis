set @period = '2016 12';
set @stream = 'MS';
set @regionLike = 'China%';
set @noOfPastPeriodsToUseInForecast = 3;
set @region = 'APAC';
set @noOfMonths = 6;
...
select analytics.getRegionFromCountry(ad.`Country`) as 'Region', ad.`Country`, ad.`Revenue Stream`, ad.`forecast period`, ad.`Available Days`/forecast.`Forecast Factor` as 'Forecast' 
from 
# Current (latest) available days for period
(select `Region`, if(`Region` like 'Australia%', 'Australia',`Region`) as 'Country', `Revenue Stream`, fp.`forecast period`, sum(Value) as 'Available Days' 
...
where 
(`Region` like 'Australia%' or `Region` in ('Asia Regional Desk', 'China', 'India', 'Indonesia', 'Japan', 'Korea', 'Thailand') )
...
left join
# Forecasting factor
(select t1.`Country`, t1.`Revenue Stream`, t1.`forecast period`, avg(t1.`Available Days`/t2.`Final Confirmed`) as 'Forecast Factor'
from (
select if(`Region` like 'Australia%', 'Australia',`Region`) as 'Country', `Revenue Stream`, str_to_date(`Report Date-Time`,'%d/%m/%Y - %T') as 'Snapshot Date', fp.`forecast period`, `Period`, sum(Value) as 'Available Days' 
...
left join ( 
select if(`Region` like 'Australia%', 'Australia',`Region`) as 'Country', if(RowName like '%Food%', 'Food', if(RowName like '%Product%', 'PS', 'MS')) as `Revenue Stream`, ColumnName as `Period`, sum(value) as 'Final Confirmed' 
...
and `Region` not like '%Product%'
and (`Region` like 'Australia%' or `Region` in ('Asia Regional Desk', 'China', 'India', 'Indonesia', 'Japan', 'Korea', 'Thailand') )
...
and  analytics.getRegionFromCountry(if(rh.`Region` like 'Australia%', 'Australia', rh.`Region`) ) = @region
...
(select 
 ...
 analytics.getRegionFromCountry(if(Region like '%Australia%', 'Australia', Region)) as 'Region',
...
where 
 ...
 and analytics.getRegionFromCountry(if(Region like '%Australia%', 'Australia', Region)) = @region
 and current=1 
group by `Region`, `Country`, `Stream`, `Period`, `Audit Status`, `Audit Open SubStatus`)
union all
(select now() as 'Report Date', analytics.getRegionFromCountry(ad.`Country`) as 'Region', ad.`Country`, ad.`Revenue Stream` as 'Stream', 'Forecast Calc' as 'Audit Status', '' as 'Audit Open SubStatus', 'Forecast Calc' as 'Simple Status', ad.`forecast period` as 'Period', ad.`Available Days`/forecast.`Forecast Factor` as 'today', 0 as 'yesterday', 0 as 'week start', 0 as 'month start' 
from 
(select `Region`, if(`Region` like 'Australia%', 'Australia',`Region`) as 'Country', `Revenue Stream`, fp.`forecast period`, sum(Value) as 'Available Days' 
from salesforce.financial_visisbility_latest, (select date_format(wd.`date`, '%Y %m') as 'forecast period' from salesforce.sf_working_days wd where date between now() and date_add(now(), interval 6 month) group by `forecast period`) fp
where (`Region` like 'Australia%' or `Region` in ('Asia Regional Desk', 'China', 'India', 'Indonesia', 'Japan', 'Korea', 'Thailand') ) and `Region` not like '%Product%' and `Period` = fp.`forecast period`  and `Audit Status` not in ('Cancelled') and (`Audit Open SubStatus` not in ('Pending Cancellation', 'Pending Suspension') or `Audit Open SubStatus` is null)
group by `Country`, `Revenue Stream`, fp.`forecast period`) as ad
left join
(select t1.`Country`, t1.`Revenue Stream`, t1.`forecast period`, avg(t1.`Available Days`/t2.`Final Confirmed`) as 'Forecast Factor' from (
select if(`Region` like 'Australia%', 'Australia',`Region`) as 'Country', `Revenue Stream`, str_to_date(`Report Date-Time`,'%d/%m/%Y - %T') as 'Snapshot Date', fp.`forecast period`, `Period`, sum(Value) as 'Available Days' 
from salesforce.financial_visisbility, (select date_format(wd.`date`, '%Y %m') as 'forecast period' from salesforce.sf_working_days wd where date between now() and date_add(now(), interval 6 month) group by `forecast period`) fp
where (`Region` like 'Australia%' or `Region` in ('Asia Regional Desk', 'China', 'India', 'Indonesia', 'Japan', 'Korea', 'Thailand') ) and `Region` not like '%Product%' and `Period` < fp.`forecast period` and `Period` > date_format(date_Add(now(), interval -4 month), '%Y %m') and `Period` < date_format(now(), '%Y %m') and `Audit Status` not in ('Cancelled') and (`Audit Open SubStatus` not in ('Pending Cancellation', 'Pending Suspension') or `Audit Open SubStatus` is null) and datediff(str_to_date(concat(`Period`, ' 01'), '%Y %m %d'), str_to_date(`Report Date-Time`,'%d/%m/%Y - %T')) = datediff(str_to_date(concat(fp.`forecast period`,' 01'), '%Y %m %d'),now()) 
group by `Country`,`Revenue Stream`, `Snapshot Date`, fp.`forecast period`, `Period` order by `Country`,`Revenue Stream`, `Period` ) t1 
left join ( 
select if(`Region` like 'Australia%', 'Australia',`Region`) as 'Country', if(RowName like '%Food%', 'Food', if(RowName like '%Product%', 'PS', 'MS')) as `Revenue Stream`, ColumnName as `Period`, sum(value) as 'Final Confirmed' from salesforce.sf_report_history 
where ReportName='Audit Days Snapshot' and Date = (select max(Date) from salesforce.sf_report_history where ReportName='Audit Days Snapshot') and `Region` not like '%Product%' and (`Region` like 'Australia%' or `Region` in ('Asia Regional Desk', 'China', 'India', 'Indonesia', 'Japan', 'Korea', 'Thailand') ) and ColumnName < date_format(now(), '%Y %m') and (RowName not like '%Open%' and RowName not like '%Service Change%' and RowName not like '%Scheduled%' and RowName not like '%Cancelled%') 
group by `Country`,`Revenue Stream`,`Period`) t2 on t2.`Period` = t1.`Period` and t2.`Country` = t1.`Country` and t2.`Revenue Stream` = t1.`Revenue Stream`
group by t1.`Country`, t1.`Revenue Stream`, t1.`forecast period`) forecast on ad.Country = forecast.Country and ad.`Revenue Stream` = forecast.`Revenue Stream` and ad.`forecast period`= forecast.`forecast period`
group by ad.`Country`, ad.`Revenue Stream`, ad.`forecast period`)
union all
(select 
 CreateDate as 'Report Date',
 analytics.getRegionFromCountry(if(Region like '%Australia%', 'Australia', Region)) as 'Region',
 if(Region like '%Australia%', 'Australia', Region) as 'Country', 'TIS' AS 'Stream', RefName as 'Audit Status', 'PFY Actual' as 'Audit Open SubStatus', '' as 'Simple Status',  date_format(date_add(RefDate, interval 1 year), '%Y-%m') as 'Period', sum(RefValue) as 'today' , 0 as 'yesterday', 0 as 'week start', 0 as 'month start'
from salesforce.sf_data 
where 
 DataType = 'Peoplesoft' 
...
union all
(select 
 t.Date as 'Report Date', 'APAC' as 'Region', 'Australia' as 'Country', 'TIS' as 'Stream', 'eLearning' as 'Audit Status', 'Actual' as 'Audit Open SubStatus', '' as 'Simple Status', date_format(t.Date, '%Y-%m') as 'Period', sum(t.Amount) as 'today', 0 as 'yesterday', 0 as 'week start', 0 as 'month start'
from (
(select 
i.Id,
...
union all
(select `Class_Begin_Date__c` as 'Report Date', 'APAC' as 'Region', 'Australia' as 'Country', 'TIS' as 'Stream', 'public' as 'Audit Status', 'Actual' as 'Audit Open SubStatus', '' as 'Simple Status', date_format(Class_Begin_Date__c, '%Y-%m') as 'Period', Amount as 'today', 0 as 'yesterday', 0 as 'week start', 0 as 'month start'
from (
...
union all
(select c.Class_End_Date__c as 'Report Date', 'APAC' as 'Region', 'Australia' as 'Country', 'TIS' as 'Stream', 'inhouse' as 'Audit Status', 'Actual' as 'Audit Open SubStatus', '' as 'Simple Status', date_format(c.Class_End_Date__c, '%Y-%m') as 'Period', sum(Total_Course_Base_Price__c) as 'today', 0 as 'yesterday', 0 as 'week start', 0 as 'month start'
...
left join salesforce.sf_data d on date_format(d.RefDate, '%Y-%m') = date_format(wd.date, '%Y-%m')
where d.DataType='Training'  and d.DataSubType in ('eLearning', 'public', 'inhouse')  and d.RefName='budget'  and wd.Date >= @fy_start  and wd.date <= @fy_end
 order by wd.date)
union all
