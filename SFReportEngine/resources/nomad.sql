(select 
    ...
    ifnull(scsp.External_Site_Code__c, 'TBA') as 'NFE Code',
    'TBA' as 'Category',
    s.Name as 'Standard',
    ...
    ccs.Name as 'Country',
    ifnull(site.Business_Zip_Postal_Code__c, '') as 'PostCode',
    ...
    'TBA' as 'Audit 2017',
    ifnull(ifnull((select wip.Work_Item_Date__c from salesforce.work_item__c wip where wip.Site_Certification_Standard__c = wi.Site_Certification_Standard__c and wip.Status__c not in ('Cancelled') and wip.IsDeleted = 0 and wip.Work_Item_Date__c < wi.Work_Item_Date__c and wip.Work_Item_Stage__c not in ('Follow Up') order by wip.Work_Item_Date__c desc limit 1),
		(select npa.work_item_Date__c from analytics.nomad_previous_cb_audits npa where npa.site_id = site.Id and npa.work_item_date__c<wi.work_item_date__c order by npa.work_item_date__c desc limit 1) ), 'TBA') as 'Previous Audit',
    '' as 'Comment',
    #wi.Name as 'Compass Audit Number',
    #wi.Status__c as 'Compass Status',
    if(wi.Status__c in ('Draft', 'Initiate Service','Open','Service Change','Initiate Service','Draft'),'To Be Booked', 
    if(wi.Status__c in ('Scheduled', 'Scheduled - Offered'), 'Dates Planned/Offered', 
    if(wi.Status__c in ('Confirmed', 'In Progress'), 'Audit Booked',
    if(wi.Status__c in ('Submitted', 'Under Review', 'Under Review - Rejected', 'Support'), 'Audit Done - Under Review/Support',
    if(wi.Status__c in ('Completed'), 'Audit Completed',
    if(wi.Status__c in ('Cancelled'), 'Cancelled', '?')))))) as 'Nomad Status',
    if(s.Name='Nomad Foods Europe V5 | Verification', 'Phase 1', 'Phase 2') as 'Phase',
    wi.Work_Item_Date__c as 'Audit Date',
    date_add(wi.Work_Item_Date__c, interval 10 day) as '10 Days',
    ...
    'TBA' as 'Rating',
    ifnull(max(wih.CreatedDate), '') as 'Audit Report Sent to Nomad',
