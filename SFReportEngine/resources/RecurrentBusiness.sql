...
FROM
    work_item__c wi
        INNER JOIN
    recordtype rt ON wi.RecordTypeId = rt.Id
        INNER JOIN
    work_package__c wp ON wp.Id = wi.Work_Package__c
        INNER JOIN
    certification__c c ON c.Id = wp.Site_Certification__c
        INNER JOIN
    site_certification_lifecycle__c sclc ON sclc.Work_Item__c = wi.Id
WHERE
    rt.Name = 'Audit'
		...
		AND wi.Work_Item_Date__c >= '2014-07-01'
		AND c.Status__c = 'Active'
		AND wi.Status__c NOT IN ( 'Cancelled')
ORDER BY `BusinessUnit` , c.Id , `Period`;
...
where 
	wi.Work_Item_Date__c>='2013-01-01' 
	and wi.Work_Item_Date__c<='2013-12-31'
	and rt.Name = 'Audit'
	AND c.Operational_Ownership__c IN ('AUS - Management Systems' , 'AUS - Food', 'AUS - Product Services')
	AND wi.Work_Package_Type__c = 'Initial'
	AND c.Status__c = 'Active'
...
# Registration Fees
select ig.CurrencyIsoCode, c.Registration_Fee_Product__c 
from salesforce.invoice_group__c ig 
left join salesforce.certification__c c on c.Invoice_Group_Registration__c = ig.Id
where ig.RecordTypeName__c='Registration'
and ig.Next_Invoice_Date__c>='2014-07-01' and ig.Next_Invoice_Date__c<='2015-06-30'
and c.Registration_Fee_Product__c is null and ig.Invoice_as_Lump_Sum__c=false
;