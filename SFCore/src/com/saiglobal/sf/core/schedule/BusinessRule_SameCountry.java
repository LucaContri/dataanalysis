...
public class BusinessRule_SameCountry extends AbstractBusinessRule {
	
	private static final Logger logger = Logger.getLogger(BusinessRule_SameCountry.class);
	public BusinessRule_SameCountry(DbHelper db) {
		super(db);
	}
	...
	public HashMap<String, Resource> filter(WorkItem workItem, HashMap<String, Resource> resourceIdWithScore) throws SQLException, ClassNotFoundException, IllegalAccessException, InstantiationException {
		...
		Utility.startTimeCounter("BusinessRule_SameCountry");
		...
		Utility.stopTimeCounter("BusinessRule_SameCountry");
