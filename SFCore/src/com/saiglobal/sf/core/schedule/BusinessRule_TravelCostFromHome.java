...
public class BusinessRule_TravelCostFromHome extends AbstractBusinessRule {
	...
	private static final Logger logger = Logger.getLogger(BusinessRule_TravelCostFromHome.class);
	public BusinessRule_TravelCostFromHome(DbHelper db) {
		super(db);
	}
	...
	public HashMap<String, Resource> filter(WorkItem workItem, HashMap<String, Resource> resourceIdWithScore) throws SQLException, ClassNotFoundException, IllegalAccessException, InstantiationException, GeoCodeApiException {
		...
		Utility.startTimeCounter("BusinessRule_TravelCostFromHome");
		...
		query = "SELECT " +
				"r.Id as 'id', " +
				"r.Managing_Office__c as 'Managing_Office', " +
		...
		*/
		query = "SELECT "
				+ "r1.Id as 'id',"
				+ "r1.Managing_Office__c as 'Managing_Office',"
				+ "DISTANCE(" + workItem.getClientSite().getLatitude() + ", " + workItem.getClientSite().getLongitude() + ",cr.Latitude,cr.Longitude) as 'distance_home' "
						+ "FROM "
						+ "( SELECT r.Id, r.Managing_Office__c, r.FLocation_Home__c "
						+ "FROM resource__c r "
						+ db.getWhereClause(whereClauseList) + ") r1 "
						+ "INNER JOIN "
						+ "saig_geocode_cache cr   ON cr.Address = r1.FLocation_Home__c";
		...
		if (emptyInput) {
			while (rs.next()) {
				...
				updateScore(resource, workItem, rs.getDouble("distance_home"), 0, SfSaigOffice.getValueForName(rs.getString("Managing_Office")));
				filteredResources.put(rs.getString("id"), resource);
			}
		} else {
			while (rs.next()) {
				Resource resource = resourceIdWithScore.get(rs.getString("id"));
				//updateScore(resource, workItem, rs.getDouble("distance_home"), rs.getDouble("distance_office"), SfSaigOffice.getValueForName(rs.getString("r.Managing_Office__c")));
				updateScore(resource, workItem, rs.getDouble("distance_home"), 0, SfSaigOffice.getValueForName(rs.getString("Managing_Office")));
				filteredResources.put(rs.getString("id"), resource);
			}
		...
		Utility.stopTimeCounter("BusinessRule_TravelCostFromHome");
	...
	private void updateScore(Resource resource, WorkItem workItem, double distance_home, double distance_office, SfSaigOffice office) throws SQLException, ClassNotFoundException, IllegalAccessException, InstantiationException, GeoCodeApiException {
		...
		double carTravelTime = Utility.addDrivingFatiguePolicyBreaks(distance_home/driving_average_speed);
		resource.setHomeDistanceFromClient(distance_home);
		if (office.equals(SfSaigOffice.Unknown)) {
			logger.error("Office Unknown for " + resource.getId() + ".  Cannot calculate flying time.  Assuming driving");
			// Driving
		...
		if (distance_home-metroAreaRadius<=workItem.getDistanceToClosestAirport(db)){
			// Driving
			resource.setTravelType(TravelType.DRIVE);
			resource.setTravelTime(carTravelTime);
			resource.setTravelCost(carTravelTime*loss_revenue_hourly_rate);
			...
			if (flyTravelTime<0) {
				...
				if (flyTravelTime > carTravelTime) {
					...
					resource.setScore(new Double(resource.getScore() + flyTravelTime*loss_revenue_hourly_rate + calculateFlightCost(distance_home)) + calculateAccomodationCost(flyTravelTime, workItem.getRequiredDuration()));
				}
			}
		}
	}
