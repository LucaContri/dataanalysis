...
public class FixedCachedRowSetImpl extends CachedRowSetImpl {
    ...
    private HashMap<String, Integer> columnsNamesMap = new HashMap<String, Integer>();
    ...
    private void initColumnsNames(ResultSet rs) throws SQLException {
    	ResultSetMetaData rsmd = rs.getMetaData();
        int cols = rsmd.getColumnCount();

        for (int i = 1; i <= cols; ++i) {
            columnsNamesMap.put(rsmd.getColumnLabel(i).toLowerCase(), i);
        }
    }
    
    private int getColIdxByName(String name) throws SQLException {
    	String localName = new String(name).toLowerCase();
    	if (localName.indexOf(".")>=0) {
    		localName = localName.substring(localName.indexOf(".")+1);
    	}
    	
    	if (columnsNamesMap.get(localName) == null)
    		throw new SQLException(localName + " is an invalid column name");
    	return columnsNamesMap.get(localName.toLowerCase());
    	/*
