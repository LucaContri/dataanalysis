...
public class SFConnection implements Connection {
	private SFStatement st;
	private GlobalProperties gp;
	private PartnerConnection connection;
	private ConnectorConfig config;
	private static final Logger logger = Logger.getLogger(SFConnection.class);
	
	public SFConnection(GlobalProperties gp) throws SQLException {
		this.gp = gp;
		try {
			connect();
	...
	private void connect() throws ConnectionException {
		...
		config.setUsername(gp.getSfUser());
		config.setPassword(gp.getSfPassword()+gp.getSfToken());
		if (gp.isEnableProxy()) {
			config.setProxy(gp.getProxyHost(), gp.getProxyPort());
			if (gp.isProxyAuthRequired()){
				config.setProxyUsername(gp.getProxyUser());
				config.setProxyPassword(gp.getProxyPassword());
			}
		}
		config.setAuthEndpoint(gp.getSfEndpoint());
		config.setReadTimeout(gp.getSfReadTimeOut());
		config.setConnectionTimeout(gp.getSfConnectionTimeout());
