...
public class ParametersListServlet extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(ParametersListServlet.class);
	private static DbHelper db;
	private static final GlobalProperties gp;
	private static ParametersCache capabilityCache;
	static {
	// Initialise
		...
		try {
			...
			db = new DbHelper(gp);
			capabilityCache = ParametersCache.getInstance(db);
		} catch (Exception e) {
			logger.error(e);
	...
	@Override
	  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			out.print(gson.toJson(capabilityCache.getParameters(q)));
		} catch (Exception e) {
