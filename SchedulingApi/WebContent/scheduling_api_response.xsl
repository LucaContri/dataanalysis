	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<xsl:variable name="previousPeriodUrl" select="ApiResponse/PreviousPeriodUrl" />
				<xsl:variable name="nextPeriodUrl" select="ApiResponse/NextPeriodUrl" />
				<xsl:choose>
					<xsl:when test="$error != ''">
						<div id="error">
																		...
																		<th>
																			<table class="days">
																				<tr>
																					...
																					<xsl:variable name="pointer2" select="position()" />
																						<th class="days">
																							<xsl:choose>
																								<xsl:when test="$pointer2 = 1">
																									<a href="{$previousPeriodUrl}">Prev.</a>&#160;&#160;&#160;<xsl:value-of select="name"/>
																								</xsl:when>
																								<xsl:when test="$pointer2 = last()">
																									<xsl:value-of select="name"/>&#160;&#160;&#160;<a href="{$nextPeriodUrl}">Next</a>
																								</xsl:when>
																								<xsl:otherwise>
